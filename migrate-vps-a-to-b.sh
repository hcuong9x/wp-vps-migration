...
if [[ -z "$SOURCE_HOST" || -z "$TARGET_HOST" || -z "$SOURCE_DOMAIN" || -z "$TARGET_DOMAIN" ]]; then
  usage
  exit 1
fi
...
echo "Step 1/4: Run backup on VPS A ($SOURCE_HOST)"
...
backup_output="$(
  run_ssh_source "${backup_cmd[@]}" < "$backup_script"
)"
printf '%s\n' "$backup_output"

backup_archive="$(printf '%s\n' "$backup_output" | awk -F= '/^BACKUP_ARCHIVE=/{print $2}' | tail -n1)"
backup_sha="$(printf '%s\n' "$backup_output" | awk -F= '/^BACKUP_SHA256=/{print $2}' | tail -n1)"
...
echo "Checksum OK: $target_sha"

echo "Step 4/4: Restore on VPS B"
